...
class unit extends CI_Controller
{
    ...
    public function process()
    {
        ...
        if (isset($_POST['add'])) {
            ...
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('success', 'Data unit successfuly added!');
            }
        } else if (isset($_POST['edit'])) {
            $this->unit_m->edit($post);
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('success', 'Data unit successfuly edited!');
            }
        }
        redirect('unit');
    }
    ...
    public function del($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('success', 'Data unit successfuly deleted!');
        }
        redirect('unit');
    }
}
