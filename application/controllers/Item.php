...
class Item extends CI_Controller
{
    ...
    public function process()
    {
        ...
        if (isset($_POST['add'])) {
            if ($this->item_m->check_barcode($post['barcode'])->num_rows() > 0) {
                $this->session->set_flashdata('error', "Barcode $post[barcode] has been used in another item!");
                redirect('item/add');
            } else {
                if (@$_FILES['image']['name'] != null) {
                    if ($this->upload->do_upload('image')) {
                        ...
                        if ($this->db->affected_rows() > 0) {
                            $this->session->set_flashdata('success', 'Data product successfuly added!');
                            redirect('item');
                        }
                    } else {
            ...
            if ($this->item_m->check_barcode($post['barcode'], $post['id'])->num_rows() > 0) {
                ...
                if (@$_FILES['image']['name'] != null) {
                    if ($this->upload->do_upload('image')) {
                        ...
                        if ($this->db->affected_rows() > 0) {
                            $this->session->set_flashdata('success', 'Data product successfuly edited!');
                            redirect('item');
                        }
                    } else {
                        $error = $this->upload->display_errors();
    ...
    public function del($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('success', 'Data item successfuly deleted!');
        }
        redirect('item');
    }
