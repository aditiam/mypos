...
class Category extends CI_Controller
{
    ...
    public function process()
    {
        ...
        if (isset($_POST['add'])) {
            ...
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('success', 'Data category successfuly added!');
            }
        } else if (isset($_POST['edit'])) {
            $this->category_m->edit($post);
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('success', 'Data category successfuly edited!');
            }
        }
        redirect('category');
    }
    ...
    public function del($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('success', 'Data category successfuly deleted!');
        }
        redirect('category');
    }
}
